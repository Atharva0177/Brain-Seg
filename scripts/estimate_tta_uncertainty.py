"""Estimate test-time augmentation uncertainty for selected subjects."""
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/uncertainty-tta.json"))
    args = parser.parse_args()
    manifest = json.loads(args.manifest.read_text(encoding="utf-8"))
    selected = json.loads(args.training_result.read_text(encoding="utf-8"))["selected"]
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    results = [
        tta_uncertainty_for_subject(
            manifest,
            selected,
            args.checkpoint,
            subject_id,
